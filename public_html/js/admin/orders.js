...
$(document).on('click', '.go_print', function(e) {
	e.preventDefault();
	var href = document.location.href;
	var clear = href.replace('#','');
	// console.log(clear)
	var new_href = clear + '&goprint=reload';
...
function init()
{
	...
	for(i = 0; i < params.length; i ++) {
		if(params[i].split("=")[0] == 'goprint' && params[i].split("=")[1] == 'reload') {
			window.print();
			var href = document.location.href
			new_ = href.replace('&goprint=reload', '');
			history.pushState('', '', new_)
			return true;
		}
	}	
